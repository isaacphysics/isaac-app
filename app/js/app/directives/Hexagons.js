...
define(["../honest/hexagon"],function(hexagon) {
    ...
    let draw = function(hex, questions, $state, boardId)
    {
        // Plot
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                let update = function() {
                    ...
                    let _width = hexWidth(); // This should be recalculated every time the window is resized
                    let hex = hexagon.calculateAndPositionHexagons($('.hexagon_wrap'), _pad, _width, _aspect, augmentedQuestions || [], equalRows);
                    ...
                    if (hexs.size() > 0 && $("footer.hide-for-small-only").is(":visible")) {
                        ...
                        let footerTop = $("footer:visible").offset().top;

                        minLeft = minLeft % 235;

                        let fromFooterTop = (footerTop - maxTop);

                        let offsety = (fromFooterTop % 193);

                        offsety -= 193;

                        let s = "left top, left " + (minLeft - 123) + "px bottom " + Math.round($("footer:visible").outerHeight() + offsety) + "px";
                ...
                scope.$watch("questions", function() {
                    if (scope.questions) {
                        ...
                        $.each(augmentedQuestions, function(i, q) {
                            ...
                            let deepestTag = tags.getDeepestTag(q.tags);
                        ...
                        let augmentedWildcard = JSON.parse(JSON.stringify(scope.wildCard));
