                    ...
                    "body": {
                        ...
                        controller: ["$scope", "api", function($scope, api) {
                            ...
                            api.statisticsEndpoint.get().$promise.then(function(result) {
                                ...
                                $scope.statistics.totalUsers = 0;
                                $scope.statistics.activeUsers = {};
                                $scope.statistics.answeringUsers = {};
                                $scope.statistics.gender = {};
                                $scope.statistics.role = {};
                                $scope.statistics.hasSchool = 0;
                                $scope.statistics.hasNoSchool = 0;
                                $scope.statistics.hasSchoolOther = 0;

                                for (var i in result.userGenders) {
                                    $scope.statistics.totalUsers += result.userGenders[i].count;
                                    $scope.statistics.gender[result.userGenders[i].gender] = result.userGenders[i].count;
                                }
                                for (var i in result.userRoles) {
                                    $scope.statistics.role[result.userRoles[i].role] = result.userRoles[i].count;
                                }

                                $scope.statistics.activeUsers.sevenDays = {};
                                $scope.statistics.activeUsers.sevenDays.total = 0;
                                for (var i in result.activeUsersLastSevenDays) {
                                    $scope.statistics.activeUsers.sevenDays[result.activeUsersLastSevenDays[i].role] = result.activeUsersLastSevenDays[i].count;
                                    $scope.statistics.activeUsers.sevenDays.total += result.activeUsersLastSevenDays[i].count;
                                }
                                $scope.statistics.activeUsers.thirtyDays = {};
                                $scope.statistics.activeUsers.thirtyDays.total = 0;
                                for (var i in result.activeUsersLastThirtyDays) {
                                    $scope.statistics.activeUsers.thirtyDays[result.activeUsersLastThirtyDays[i].role] = result.activeUsersLastThirtyDays[i].count;
                                    $scope.statistics.activeUsers.thirtyDays.total += result.activeUsersLastThirtyDays[i].count;
                                }
                                $scope.statistics.activeUsers.ninetyDays = {};
                                $scope.statistics.activeUsers.ninetyDays.total = 0;
                                for (var i in result.activeUsersLastNinetyDays) {
                                    $scope.statistics.activeUsers.ninetyDays[result.activeUsersLastNinetyDays[i].role] = result.activeUsersLastNinetyDays[i].count;
                                    $scope.statistics.activeUsers.ninetyDays.total += result.activeUsersLastNinetyDays[i].count;
                                }

                                $scope.statistics.answeringUsers.sevenDays = {};
                                $scope.statistics.answeringUsers.sevenDays.total = 0;
                                for (var i in result.answeringUsersLastSevenDays) {
                                    $scope.statistics.answeringUsers.sevenDays[result.answeringUsersLastSevenDays[i].role] = result.answeringUsersLastSevenDays[i].count;
                                    $scope.statistics.answeringUsers.sevenDays.total += result.answeringUsersLastSevenDays[i].count;
                                }
                                $scope.statistics.answeringUsers.thirtyDays = {};
                                $scope.statistics.answeringUsers.thirtyDays.total = 0;
                                for (var i in result.answeringUsersLastThirtyDays) {
                                    $scope.statistics.answeringUsers.thirtyDays[result.answeringUsersLastThirtyDays[i].role] = result.answeringUsersLastThirtyDays[i].count;
                                    $scope.statistics.answeringUsers.thirtyDays.total += result.answeringUsersLastThirtyDays[i].count;
                                }
                                $scope.statistics.answeringUsers.ninetyDays = {};
                                $scope.statistics.answeringUsers.ninetyDays.total = 0;
                                for (var i in result.answeringUsersLastNinetyDays) {
                                    $scope.statistics.answeringUsers.ninetyDays[result.answeringUsersLastNinetyDays[i].role] = result.answeringUsersLastNinetyDays[i].count;
                                    $scope.statistics.answeringUsers.ninetyDays.total += result.answeringUsersLastNinetyDays[i].count;
                                }

                                for (var i in result.userSchools) {
                                    if (result.userSchools[i].has_school_id) {
                                        $scope.statistics.hasSchool += result.userSchools[i].count;
                                    } else if (result.userSchools[i].has_school_other && !result.userSchools[i].has_school_id) {
                                        $scope.statistics.hasSchoolOther += result.userSchools[i].count;
                                    } else {
                                        $scope.statistics.hasNoSchool += result.userSchools[i].count;
                                    }
                                }
                                $scope.statistics.activeInLastSixMonths = result.activeInLastSixMonths.count;
                                $scope.statistics.viewQuestionEvents = result.viewQuestionEvents.count;
                                $scope.statistics.answeredQuestionEvents = result.answeredQuestionEvents.count;
                                $scope.statistics.groupCount = result.groupCount.count;
